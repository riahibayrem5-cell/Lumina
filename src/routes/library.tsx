...
export const Route = createFileRoute("/library")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your reading progress, uploads, and AI summaries." },
      { property: "og:title", content: "My Library — Lumina Books" },
      { property: "og:description", content: "Your reading progress, uploads, and AI summaries." },
    ],
  }),
      ...
      <div className="mx-auto w-full max-w-7xl px-6 py-12">
        ...
        >
          ...
          <p className="mt-4 max-w-xl font-serif italic text-espresso/70">
            Curated classics, your private uploads, and quick title-only summaries — all in one place.
          </p>
        </motion.div>
    ...
    <section>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {entries.map((entry) => {
          ...
          const isUpload = entry.bookId.startsWith("upload-");
          if (!book && !isUpload) return null;
          const total = entry.totalChapters ?? 1;
          const ratio = Math.min(1, (entry.chapter + entry.scrollRatio) / Math.max(1, total));
          const displayTitle = book?.title ?? "Personal upload";
          const displayAuthor = book?.author ?? "Your library";
          return (
            <div
              key={entry.bookId}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <Link
                  ...
                  params={{ bookId: entry.bookId, chapter: String(entry.chapter) }}
                  className="block"
                >
                  ...
                  <p className="text-xs uppercase tracking-wider text-muted-foreground">
                    {displayAuthor} · ch. {entry.chapter + 1}
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {(["reading", "to-read", "completed", "paused"] as const).map((s) => (
                    <button
                      key={s}
                      onClick={() => onStatus(entry.bookId, s)}
